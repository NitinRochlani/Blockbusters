...
train_X_df['release_year'] = pd.to_datetime(train_df['release_date']).dt.year
train_X_df['release_year'] = train_X_df['release_year'].apply(lambda x: 1980 if pd.isna(x) else x)

test_X_df['release_year'] = pd.to_datetime(test_df['release_date']).dt.year
test_X_df['release_year'] = test_X_df['release_year'].apply(lambda x: 1980 if pd.isna(x) else x)

#bins = [1960, 1970, 1980, 1990,2000,2010,2020,2030,2040,2050]
#labels = ['60s', '70s', '80s', '90s','00s','10s','20s','30s','40s']
#decades = pd.cut(train_X_df.release.dt.year, bins, labels=labels)
#dates_df['decades'] = decades
...
test_y_df.to_csv('submission_7.csv')
